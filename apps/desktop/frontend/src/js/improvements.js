...
export function renderImprovementProposals(proposals = []) {
  ...
  return `<div class="improvement-grid">${proposals.map((proposal) => {
    ...
    const eligible = metrics.passed !== false && (!shadow || (Number(shadow.samples) >= Number(shadow.minimumSamples) && Number(shadow.permissionDelta || 0) <= 0));
    const failures = Array.isArray(metrics.failures) && metrics.failures.length ? `<ul class="improvement-failures">${metrics.failures.map((failure) => `<li>${escapeHtml(failure)}</li>`).join("")}</ul>` : "";
