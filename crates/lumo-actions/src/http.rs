...
impl Action for RequestAction {
    fn id(&self) -> &'static str { "http.request" }
    fn summary(&self) -> &'static str { "Make an HTTP request and return status/body/headers" }
    async fn execute(&self, _ctx: &mut StepCtx, input: Value) -> Result<ActionResult, StepError> {
        let ReqIn { method, url, headers, query, body, timeout_ms } =
        ...
        let mut req = client
            .request(
                method.parse().map_err(|e| StepError::msg(format!("bad method: {e}")))?,
                &url,
            )
            .query(&query);
        ...
        let resp_headers: HashMap<_, _> = resp.headers().iter()
