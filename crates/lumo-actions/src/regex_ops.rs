...
impl Action for MatchAction {
    ...
    fn summary(&self) -> &'static str { "Return true when `pattern` matches any part of `text`" }
    fn schema(&self) -> &'static Value { pat_text_schema() }
...
impl Action for FindAllAction {
    fn id(&self) -> &'static str { "regex.find_all" }
    fn summary(&self) -> &'static str { "Return every non-overlapping match as a string array" }
    fn schema(&self) -> &'static Value { pat_text_schema() }
    async fn execute(&self, _ctx: &mut StepCtx, input: Value) -> Result<ActionResult, StepError> {
        let PatTextIn { pattern, text } = serde_json::from_value(input)
...
impl Action for ReplaceAction {
    fn id(&self) -> &'static str { "regex.replace" }
    fn summary(&self) -> &'static str { "Replace pattern with `replacement` (supports $1, $2 capture refs)" }
    ...
    async fn execute(&self, _ctx: &mut StepCtx, input: Value) -> Result<ActionResult, StepError> {
        let ReplaceIn { pattern, text, replacement, once } = serde_json::from_value(input)
...
impl Action for CapturesAction {
    fn id(&self) -> &'static str { "regex.captures" }
    fn summary(&self) -> &'static str { "Return first match's groups: { full, groups[], named{} }" }
    fn schema(&self) -> &'static Value { pat_text_schema() }
    async fn execute(&self, _ctx: &mut StepCtx, input: Value) -> Result<ActionResult, StepError> {
        let PatTextIn { pattern, text } = serde_json::from_value(input)
