...
impl SkillRegistry {
    ...
    pub fn insert(&self, skill: Skill) {
        self.by_name.lock().insert(skill.name().to_string(), Arc::new(skill));
    }
    ...
    pub fn load_dir(&self, root: impl AsRef<Path>) -> Result<usize, crate::loader::LoadError> {
        ...
        let n = skills.len();
        for s in skills { g.insert(s.name().to_string(), Arc::new(s)); }
        Ok(n)
    }

    /// `~/.lumorpa/skills` (or env override).
    pub fn default_root() -> PathBuf {
        if let Ok(p) = std::env::var("LUMO_SKILLS_PATH") { return PathBuf::from(p); }
