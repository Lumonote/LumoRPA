...
impl Action for InvokeAction {
    fn id(&self) -> &'static str { "skill.invoke" }
    fn summary(&self) -> &'static str { "Invoke a registered Skill (sub-flow)" }
    async fn execute(&self, ctx: &mut StepCtx, input: Value) -> Result<ActionResult, StepError> {
        ...
        let report = vm.run(&skill.flow, RunOptions {
            inputs,
            trigger_kind: format!("skill:{}", name),
        }).await.map_err(|e| StepError::msg(format!("skill `{name}`: {e}")))?;
